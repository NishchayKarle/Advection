...
double first_order_val(double i_j, double in_j, double ip_j, double i_jn, double i_jp,
...
#pragma omp parallel for num_threads(16)
	for (int i = 1; i < cols - 1; i++)
		for (int j = 1; j < cols - 1; j++)
			arr2[i][j] = first_order_val(arr[i][j], arr[i + 1][j], arr[i - 1][j],
...
#pragma omp parallel for num_threads(16)
	...
	{
		// TOP
		arr2[0][i] = first_order_val(arr[0][i], arr[1][i], arr_up[i], arr[0][i + 1], arr[0][i - 1], u, v, dt, dx);

		// BOTTOM
		arr2[cols - 1][i] = first_order_val(arr[cols - 1][i], arr_down[i], arr[cols - 2][i], arr[cols - 1][i + 1],
											arr[cols - 1][i - 1], u, v, dt, dx);

		// LEFT
		arr2[i][0] = first_order_val(arr2[i][0], arr[i + 1][0], arr[i - 1][0], arr[i][1], arr_left[i], u, v, dt, dx);

		// RIGHT
		arr2[i][cols - 1] = first_order_val(arr2[i][cols - 1], arr[i + 1][cols - 1], arr[i - 1][cols - 1],
...
void update_corners(int cols, double arr2[cols][cols], double arr[cols][cols], double arr_up[cols],
					...
					double u, double v, double dt, double dx)
{
	arr2[0][0] = first_order_val(arr[0][0], cols == 1 ? arr_down[0] : arr[1][0], arr_up[0],
								 cols == 1 ? arr_right[0] : arr[0][1], arr_left[0], u, v, dt, dx);

	arr2[cols - 1][0] = first_order_val(arr[cols - 1][0], arr_down[0], cols == 1 ? arr_up[0] : arr[cols - 2][0],
										cols == 1 ? arr_right[0] : arr[cols - 1][1],
										arr_left[cols - 1], u, v, dt, dx);

	arr2[0][cols - 1] = first_order_val(arr[0][cols - 1], cols == 1 ? arr_down[0] : arr[1][cols - 1], arr_up[cols - 1],
										arr_right[0], cols == 1 ? arr_left[0] : arr[0][cols - 2],
										u, v, dt, dx);

	arr2[cols - 1][cols - 1] = first_order_val(arr[cols - 1][cols - 1], arr_down[cols - 1],
...
void first_order(int N, int NT, double L, double T, double u, double v, double dx, double dt)
{
...
int main(int argc, char **argv)
{
	if (argc == 7)
	{
		...
		first_order(N, NT, L, T, u, v, dx, dt);
