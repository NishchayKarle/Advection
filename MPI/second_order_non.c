    ...
    for (int i = 2; i < cols + 2; i++)
        for (int j = 2; j < cols + 2; j++)
            arr2[i][j] = sec_order_val(arr[i][j], arr[i + 1][j], arr[i + 2][j], arr[i - 1][j],
                                       ...
                                       arr[i][j - 2], 2 * u + u / (start_i + i + 1), 2 * v + v / (start_j + j + 1), dx, dx, dt);
}
// u + u + u / (start_i + i + 1), v + v + v / (start_j + j + 1)
...
void second_order_upwind(int N, int NT, double L, double T, double u, double v, double dx, double dt)
{
    ...
    start_j = CHUNK_SIZE * (task_id % (int)sqrt(num_tasks)),
    start_i = task_id / sqrt(num_tasks);
    start_i *= CHUNK_SIZE;
